...
userRouter.post("/:userId/purchaseHistory", async (req, res, next) => {
  try {
    ...
    if (purchasedBook) {
      ...
      } // purchasedBook (and EVERYTHING you get from .find() .findById() .findOne()) is a MONGOOSE DOCUMENT (special objects with lots of strange fields), it is NOT A NORMAL OBJECT. Therefore if I want to spread it I shall use .toObject(), which converts a document into a PLAIN OBJECT
...
userRouter.get("/:userId/purchaseHistory", async (req, res, next) => {
  try {
    const user = await UsersModel.findById(req.params.userId)
    if (user) {
    ...
    try {
      ...
      if (user) {
        const purchasedBook = user.purchaseHistory.find(
          (book) => req.params.productId === book._id.toString()
        ) // You CANNOT compare a string (req.params.productId) with an ObjectId (book._id) --> we shall convert ObjectId into a string
        if (purchasedBook) {
          res.send(purchasedBook)
        } else {
          next(
            createError(404, `Book with id ${req.params.productId} not found!`)
          )
        }
      } else {
        next(createError(404, `User with id ${req.params.userId} not found!`))
...
userRouter.post("/:userId/cart", async (req, res, next) => {
  try {
    ...
    if (!purchasedBook)
      return next(createError(404, `Book with id ${bookId} not found!`))
